...
export default function NewDonationPage() {
  ...
  const handleSubmit = async () => {
    if (!selectedOrganization || !deliveryType || !selectedSlot || selectedItems.length === 0) {
      toast({
        title: "Error",
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-main)]">
      <div className="bg-white border-b border-[var(--input-border)] p-4 flex items-center gap-3">
        <button onClick={() => router.push('/dashboard/donate')} className="p-2 hover:bg-gray-100 rounded-full">
          <ArrowLeft className="h-5 w-5" />
        </button>
        <h1 className="text-xl font-bold text-[var(--text-primary)]">Add New Donation Request</h1>
      </div>
