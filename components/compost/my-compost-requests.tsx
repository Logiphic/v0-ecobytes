...
export function MyCompostRequests() {
  const { toast } = useToast()
  const [requests, setRequests] = useState<CompostRequest[]>([])
  const [filteredRequests, setFilteredRequests] = useState<CompostRequest[]>([])
  const [searchQuery, setSearchQuery] = useState("")
  const [loading, setLoading] = useState(true)
  const [selectedRequest, setSelectedRequest] = useState<CompostRequest | null>(null)
  ...
  useEffect(() => {
    if (searchQuery) {
      const filtered = requests.filter((req) =>
        ...
        req.farm.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const fetchRequests = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .select(`
          *,
          farm:farms(name, phone, email, address),
          slot:farm_slots(date, time)
        `)
        .eq('user_id', user.id)
  ...
  const handleContact = (request: CompostRequest, method: 'phone' | 'email') => {
    if (method === 'phone') {
      window.open(`tel:${request.farm.phone}`)
    } else {
      window.open(`mailto:${request.farm.email}`)
    }
  }

  const openDetails = (request: CompostRequest) => {
    ...
    <>
      <div className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="flex items-center justify-between p-4">
          ...
          <h1 className="text-xl font-bold text-[var(--text-primary)] absolute left-1/2 -translate-x-1/2">Compost</h1>
          <div className="w-10" />
        </div>
      </div>
          ...
          {selectedRequest && (
            <div className="mt-6 space-y-6 pb-6 overflow-y-auto max-h-[calc(85vh-120px)]">
              <div className="space-y-4 bg-white rounded-2xl p-4 border border-gray-200">
                ...
                <div>
                  <span className="text-xs font-medium text-[var(--text-muted)] uppercase">Farm</span>
                  <p className="text-base font-semibold text-[var(--text-primary)] mt-1">{selectedRequest.farm.name}</p>
                </div>
                <div>
